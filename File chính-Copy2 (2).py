...
df = pd.read_csv(
    output_file,  # Sử dụng biến chứa tên tệp thực sự
...
df['Hist.'] = pd.to_numeric(df['Hist.'], errors='coerce')
invalid_names = df[df["Name"].isna() | (df["Name"] == "")] 

if not invalid_names.empty:
    df = df.dropna(subset=["Name"])  

invalid_categories = df[df["Category"].isna() | (df["Category"] == "")] 
