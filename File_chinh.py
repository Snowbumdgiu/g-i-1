...
def join_files(part_files, output_file):
    """
    Hàm ghép các file nhỏ thành file zip gốc.
    """
    try:
        with open(output_file, 'wb') as outfile:
            ...
            print(f"Đã ghép thành công file: {output_file}")
    except FileNotFoundError as e:
        print(f"Lỗi: Không tìm thấy file {e.filename}")
    except Exception as e:
        print(f"Lỗi không xác định: {e}")

# Danh sách các file part
parts = ['part_00', 'part_01']
output_file = 'Korea.zip'

# Gọi hàm ghép file
join_files(parts, output_file)
...
files = [file for file in os.listdir(txt_folder) if file.endswith('.txt') and file != 'danh_sach.txt']
files.sort()
